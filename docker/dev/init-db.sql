-- Database initialization script for WhisprCalls
...
-- DO $$
-- BEGIN
--     IF NOT EXISTS (SELECT FROM pg_catalog.pg_roles WHERE rolname = 'whispr_calls') THEN
--         CREATE ROLE whispr_calls WITH LOGIN PASSWORD 'secure_password_here';
--     END IF;
-- END
-- $$;

-- Grant necessary permissions
-- GRANT CONNECT ON DATABASE whispr_calls_dev TO whispr_calls;
-- GRANT USAGE ON SCHEMA public TO whispr_calls;
-- GRANT CREATE ON SCHEMA public TO whispr_calls;

-- Set up recommended PostgreSQL settings for call workloads
...
COMMENT ON DATABASE whispr_calls_dev IS 'WhisprMessenger calls service database';
