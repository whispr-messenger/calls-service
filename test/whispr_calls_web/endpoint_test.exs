defmodule WhisprCallsWeb.EndpointTest do
  @moduledoc """
  Tests pour les helpers endpoint (WHISPR-1354 WebSocket origin check).
  """

  use ExUnit.Case, async: false

  alias WhisprCallsWeb.Endpoint
  ...
  setup do
    ...
    on_exit(fn ->
      case original_env do
        nil -> Application.delete_env(:whispr_calls, :env)
        value -> Application.put_env(:whispr_calls, :env, value)
      end
  ...
  describe "ws_check_origin/1 in non-prod" do
    ...
    test "permits any origin in dev env" do
      Application.put_env(:whispr_calls, :env, :dev)

      assert Endpoint.ws_check_origin(URI.parse("http://localhost:4000")) == true
    end
  end
