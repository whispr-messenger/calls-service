defmodule WhisprCallsWeb.Plugs.Cors do
  @moduledoc """
  CORS headers for cross-origin requests.

  Fail-closed semantics (WHISPR-1354):
  ...
  An `OPTIONS` preflight is answered directly with `204` and the CORS
  headers, before the router (which would otherwise return 404).
  """
  ...
  defp allowed_origins do
    case {prod?(), System.get_env("CORS_ALLOWED_ORIGINS")} do
      {true, nil} ->
        raise "CORS_ALLOWED_ORIGINS must be set in production (WHISPR-1354)"

      {true, ""} ->
        raise "CORS_ALLOWED_ORIGINS cannot be empty in production (WHISPR-1354)"
  ...
  defp prod? do
    Application.get_env(:whispr_calls, :env) == :prod
  end
